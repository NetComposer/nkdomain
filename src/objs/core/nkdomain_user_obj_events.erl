...
event({status_updated, SrvId, Path, Status}, #obj_state{id=#obj_id_ext{obj_id=UserId}}=State) ->
    {event, {status_updated, UserId, Path, #{srv_id=>SrvId, status=>Status}}, State};

event({presence_updated, Type, Path, Presence}, #obj_state{id=#obj_id_ext{obj_id=UserId}}=State) ->
    {event, {presence_updated, UserId, Path, #{type=>Type, presence=>Presence}}, State};
