...
%% @doc Mail Provider Object Type View

-module(nkdomain_mail_provider_obj_type_view).
    ...
    #{
        columns => [
            ...
            #{
                ...
                options => get_agg_term(<<?DOMAIN_MAIL_PROVIDER/binary, ".class">>, Path, Session),
                sort => false % true
            },
            #{
                id => created_by,
...
fields() ->
    [
        ...
        <<"created_by">>,
        <<"enabled">>,
        <<?DOMAIN_MAIL_PROVIDER/binary, ".class">>
    ].


%% @doc
sort_field(<<"class">>) -> <<?DOMAIN_MAIL_PROVIDER/binary, ".class">>;
sort_field(_) -> <<>>.


%% @doc
filter_field(<<"class">>, Data, Acc) ->
    nkdomain_admin_util:add_filter(<<?DOMAIN_MAIL_PROVIDER/binary, ".class">>, Data, Acc);
...
get_agg_name(Field, Path, Session) ->
    nkdomain_admin_util:get_agg_name(Field, ?DOMAIN_MAIL_PROVIDER, Path, Session).



%% @private
get_agg_term(Field, Path, Session) ->
    nkdomain_admin_util:get_agg_term(Field, ?DOMAIN_MAIL_PROVIDER, Path, Session).
